...
@section('title', $destino->nombre . ' — GoWayki')
            ...
            <div class="lg:col-span-2">
                ...
                <div class="flex items-center space-x-1 text-yellow-500 text-lg mb-4">
                    @for ($i = 1; $i <= 5; $i++)
                        ...
                        @endif
                    @endfor
                    <span class="text-gray-600 ml-2">{{ number_format($destino->calificacion, 1) }}</span>
                </div>

                <p class="text-gray-700 leading-relaxed text-lg">{{ $destino->descripcion }}</p>
            </div>
                            ...
                            <form method="POST" action="{{ route('perfil.visitar') }}">
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="date" name="fecha_visita" value="{{ date('Y-m-d') }}"
                                        class="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-[#E74C3C] focus:border-transparent">
                                </div>
                                <div class="mb-4">
                                    <label class="block text-gray-600 text-sm mb-1">Notas</label>
                                    <textarea name="notas" rows="2" class="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-[#E74C3C] focus:border-transparent" placeholder="¿Qué te pareció?"></textarea>
                                </div>
...
@push('scripts')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            const mapa = L.map('mapa').setView([{{ $destino->latitud ?? '-16.4090' }}, {{ $destino->longitud ?? '-71.5375' }}], 15);
