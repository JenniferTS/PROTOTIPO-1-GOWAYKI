...
<div class="card" style="padding:24px;max-width:720px;">
    <form method="POST" action="{{ route('admin.rutas.store') }}">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;">
            <div class="form-group">
                <label>Origen</label>
                <input class="form-control" name="origen" value="{{ old('origen') }}" required>
                @error('origen')<small style="color:var(--danger);">{{ $message }}</small>@enderror
            </div>
            <div class="form-group">
                <label>Destino</label>
                <input class="form-control" name="destino" value="{{ old('destino') }}" required>
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:14px;">
            <div class="form-group">
                ...
                <input class="form-control" type="number" name="tiempo_estimado_minutos" value="{{ old('tiempo_estimado_minutos', 30) }}" min="0">
            </div>
            <div class="form-group">
                <label>Costo (S/)</label>
                <input class="form-control" type="number" step="0.10" name="costo_aproximado_soles" value="{{ old('costo_aproximado_soles', 1.00) }}" min="0">
            </div>
            <div class="form-group">
                <label>Color de línea</label>
                <input class="form-control" type="color" name="color_linea" value="{{ old('color_linea', '#F83A34') }}">
            </div>
        </div>

        <div class="form-group" style="display:flex;align-items:center;gap:8px;">
            <input type="checkbox" name="activa" value="1" id="activa" checked>
