...
<div class="card" style="padding:24px;max-width:720px;">
    <form method="POST" action="{{ route('admin.paraderos.store') }}">
        @csrf
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;">
            <div class="form-group">
                ...
                <select class="form-control" name="ruta_id" required>
                    ...
                    @foreach ($rutas as $r)
                    <option value="{{ $r->id }}" {{ old('ruta_id') == $r->id ? 'selected' : '' }}>{{ $r->nombre }} ({{ $r->origen }} → {{ $r->destino }})</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <label>Orden</label>
                <input class="form-control" type="number" name="orden" value="{{ old('orden') }}" min="0">
            </div>
        </div>

        <div class="form-group">
            <label>Referencia (opcional)</label>
            <input class="form-control" name="referencia" value="{{ old('referencia') }}" placeholder="Ej: Frente al mercado">
        </div>

        <input type="hidden" name="latitud" id="f_latitud" value="{{ old('latitud', '-16.409') }}">
        <input type="hidden" name="longitud" id="f_longitud" value="{{ old('longitud', '-71.537') }}">
        ...
        <div class="form-group">
            ...
            <p style="font-size:0.78rem;color:var(--color-text-muted);margin-top:6px;">
                Coordenadas: <span id="pCoordsLabel">-16.409, -71.537</span>
            </p>
        </div>
...
document.addEventListener('DOMContentLoaded', function () {
    const mapa = L.map('mapaParadero', { center: [-16.409, -71.537], zoom: 15 });
    ...
    const marker = L.marker(ll, { draggable: true }).addTo(mapa);
